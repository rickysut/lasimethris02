...
function polygonToJSONString(polygon) {
  ...
  // function to update polygon input value
  function updatePolyInput() {
	if (polygon) {
	  const jsonString = polygonToJSONString(polygon);
	  polyInput.value = jsonString;
	}
  }
  
  // listen to events when marker and polygon are dragged or edited
  if (marker) {
	marker.on('dragend', () => {
	  const latlng = marker.getLatLng();
	  latInput.value = latlng.lat.toFixed(6);
  ...
  polyInput.addEventListener('change', () => {
	const jsonString = polyInput.value;
	if (jsonString) {
	  const latlngs = JSON.parse(jsonString);
	  const newPolygon = L.polygon(latlngs);
	  updatePolygon(newPolygon);
	}
  });
  ...
  // add clear button to remove marker and polygon
